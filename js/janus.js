...
Janus.isExtensionEnabled = function() {
    ...
    let isShareEnabled = (!navigator.getDisplayMedia && !navigator.mediaDevices.getDisplayMedia);
		...
		if(isAudioSendEnabled(media) || isVideoSendEnabled(media)) {
			...
			if(videoSupport === true && media != undefined && media != null) {
				...
				if(media.video && media.video != 'screen' && media.video != 'window') {
					...
					// Pure-browser screen capture via the standard getDisplayMedia API.
					// (This replaces the old navigator.getDisplayMedia + Electron
					// desktopCapturer fallback from the Electron build - browsers only
					// need the mediaDevices-namespaced call below.)
					if(navigator.mediaDevices && navigator.mediaDevices.getDisplayMedia) {
						...
						pluginHandle.consentDialog(false);
						callbacks.error(new Error('Screen sharing is not supported by this browser (getDisplayMedia unavailable)'));
					}
					return;
				}
			}
